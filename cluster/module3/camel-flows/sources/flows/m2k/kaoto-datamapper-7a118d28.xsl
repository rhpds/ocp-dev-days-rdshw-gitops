    ...
    <xsl:variable name="mapped-xml">
        <map xmlns="http://www.w3.org/2005/xpath-functions">
            <string key="timestamp">
                <xsl:value-of select="$matrix-x/fn:map/fn:string[@key='sent']"/>
            </string>
            <string key="source">
                <xsl:value-of select="&quot;matrix&quot;"/>
            </string>
            <string key="user">
                <xsl:value-of select="$matrix-x/fn:map/fn:map[@key='fromUser']/fn:string[@key='displayName']"/>
            </string>
            <string key="text">
                <xsl:value-of select="$matrix-x/fn:map/fn:string[@key='text']"/>
            </string>
        </map>
    </xsl:variable>
    <xsl:template match="/">
